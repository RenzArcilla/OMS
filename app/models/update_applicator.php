<?php
/*
    This file handles the UPDATE operation for applicators.
    Updates an existing applicator record in the database.
*/
...
function updateApplicator($applicator_id, $control_no, $terminal_no, $description, 
    ...
    /*
        Update an existing applicator record in the database.

        Args:
        - $applicator_id: int, ID of the applicator to update
        - $control_no: string, control number
        - $terminal_no: string, terminal number
        - $description: string, applicator type (SIDE/END)
        - $wire_type: string, wire type (BIG/SMALL)
        - $terminal_maker: string
        - $applicator_maker: string
